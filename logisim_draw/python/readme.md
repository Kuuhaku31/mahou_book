...
<!-- | 去图  | O   | X    | O   | O   | X       | 从目标 circ 文件中去除**所有** circuit 标签下的所有像素，并保存到目标地址 | -->
<!-- | 上图（todo） | O   | circ 文件保存地址 | 源像素库文件夹路径 | 无效 | 有效    | 将像素库的**所有**像素信息添加到目标 circ 文件的相对应的 circuit 标签下 <br> 然后把更新后的 circ 文件保存到指定位置 | -->
