...
def 解析图像文件(图片地址: str, dx: int, dy: int) -> str:
    ...
    for y in tqdm(range(高), desc="处理图像行数"):  # 遍历每个像素点，添加进度条
        for x in range(宽):
            ...
            # 添加到输出结果中
            输出结果 += f'<rect fill="{hex_color}" height="1" stroke="none" width="1" x="{x+dx}" y="{y+dy}"/>\n'
...
if __name__ == "__main__":
    ...
    if 模式 == "del":
        if not 目标circ文件地址 or not 目标circuit名称:
            print("缺少参数: -t 或 -l")
            sys.exit(1)
        清除原有像素(目标circ文件地址, 目标circuit名称)

    elif 模式 == "add":
        if not 目标circ文件地址 or not 目标circuit名称 or not 待加载图像地址:
            print("缺少参数: -t、-l 或 -p")
            sys.exit(1)

        像素信息 = 解析图像文件(待加载图像地址, 50, 40)  # 这里dx, dy可根据需要调整
        添加新像素(目标circ文件地址, 目标circuit名称, 像素信息)
